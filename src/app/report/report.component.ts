...
export class ReportComponent implements OnInit {
    ...
    constructor(private ActivatedRoute: ActivatedRoute, private storage: StorageService, private hash: HashService, private leaflet: LeafletService, private titleService: TitleService, private metaService: Meta) { }
    ...
    ngOnInit(): void {
        this.storage.getObservable("reports").subscribe((data: any) => {
            ...
            if (this.index !== -1) {
                ...
                this.titleService.setTitle(`${this.reports[this.index].name} Report`);
                this.metaService.updateTag({ name: 'description', content: `${this.reports[this.index].name} is a notorious villain located in ${this.reports[this.index].location}. NCT should get him in no time, but stay alert!` });
            }
        });
